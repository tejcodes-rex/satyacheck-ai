...
class GeminiAIAnalyzer:
    ...
    def analyze_claim(self, claim: str, cultural_context: Dict = None, 
                     pib_results: List = None, language: str = 'hi') -> AIAnalysisResult:
    ...
    def _create_analysis_prompt(self, claim: str, context: Dict, language: str) -> str:
        ...
        from utils import detect_and_translate_if_needed
...
RECOMMENDED VERDICT: True (confidence 80-90%)"""
    ...
    - For claims about events in {current_year} or future years: treat as current/recent events requiring real-time verification
    ...
    {{
        ...
        "sources": ["array of credible source URLs or references when available"],
        "categories": {{
            "bias": {{
    ...
    def _parse_gemini_response(self, raw_response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not raw_response.get('success'):
            raise ValueError(f"API call failed: {raw_response.get('error', 'Unknown error')}")
...
def analyze_claim_with_ai(claim: str, cultural_context: Dict = None,
                         pib_results: List = None, target_language: str = 'hi') -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'verdict': 'Error',
            'score': 0,
            'explanation': f'Analysis failed due to system error: {str(e)}',
...
def test_ai_analyzer() -> bool:
    """Enhanced AI analyzer connectivity test"""
    try:
        analyzer = get_ai_analyzer()
        
        # Simple connectivity test
        test_claim = "This is a test claim for system connectivity"
        test_result = analyzer.analyze_claim(
            test_claim, 
            {}, 
            [], 
            'en'
        ...
        return (test_result.verdict != 'Error' and 
                test_result.confidence_score >= 0 and
                len(test_result.explanation) > 0)
