...
from config import (
    ...
    SOURCE_CREDIBILITY_SCORES, DOMAIN_EXPERTISE_BONUS,  # Add these
    is_production
)
from utils import (
...
def search_pib_factcheck(claim: str, max_results: int = 3) -> List[Dict]:
    ...
    try:
        ...
        return []
def search_realtime_web(claim: str, max_results: int = 5) -> List[Dict]:
    """Search real-time web information using Google Search API"""
    if not GOOGLE_SEARCH_API_KEY or not claim:
        return []
    
    try:
        with PerformanceTimer("realtime_web_search"):
            key_terms = extract_key_terms(claim, max_terms=3)
...
def parse_web_search_results(data: Dict, original_claim: str) -> List[Dict]:
    """Parse web search results for real-time information"""
    results = []
...
def validate_and_filter_real_sources(search_results: List[Dict], claim_text: str) -> List[str]:
        ...
        if 'claim' in request.form:
            ...
            if audio_source == "live":
                ...
                try:
                    ...
                    raise ValueError(f"Invalid live audio data: {str(e)}")
...
def extract_content_from_input(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Content extraction completed: {len(extraction_result['text'])} characters")
...
# PIB and web search in parallel
        ...
        try:
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                web_future = executor.submit(search_realtime_web, claim_text, 5)
        ...
        if not verified_sources:
            ...
            verified_defaults = []
            for default_url in default_sources:
                try:
                    response = requests.head(default_url, timeout=3,
                                           headers={'User-Agent': 'SatyaCheck-Bot/1.0'})
                    if response.status_code < 400:
                        verified_defaults.append(default_url)
                except:
                    logger.warning(f"Default source not accessible: {default_url}")
                    continue
...
def satyacheck_main(request: Request):
    """Main SatyaCheck AI endpoint for comprehensive fact-checking"""
...
def health_check(request: Request):
    ...
    try:
        ...
        try:
            ...
            services_health['database'] = f'error: {str(e)}'
            overall_healthy = False
        ...
        try:
            ...
            services_health['ai_analyzer'] = f'error: {str(e)}'
            overall_healthy = False
        ...
        try:
            ...
            services_health['content_processor'] = f'error: {str(e)}'
            overall_healthy = False
        ...
        try:
            ...
            services_health['pib_search'] = f'error: {str(e)}'
...
# DEMO ENDPOINT
@functions_framework.http
def demo_endpoint(request: Request):
    """Demo endpoint with sample data for hackathon"""
    if request.method == 'OPTIONS':
        return add_cors_headers(jsonify({}))
...
def debug_request(request: Request):
    """Debug endpoint to inspect request structure"""
    ...
    try:
        ...
        response = jsonify(create_error_response('debug_failed', str(e)))
...
logger.info("📊 PIB integration and real-time analytics enabled")