...
# Response cache for faster repeated requests (1 hour TTL)
import hashlib
...
def process_input_data(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get deep analysis flag
        input_data['deep_analysis'] = request.form.get('deep_analysis', 'false').lower() == 'true'
        logger.info(f"Deep analysis mode: {input_data['deep_analysis']}")
...
# HELPER: Get friendly error messages for extraction failures
def get_friendly_extraction_error_message(input_type: str) -> str:
    """Get user-friendly error message based on input type"""
    messages = {
        'image': 'No text found in the image. The image may be blank, contain only graphics, or the text may not be clear enough to extract.',
        'audio': 'Unable to extract speech from the audio. The audio may be silent, unclear, contain only background noise, or the speech may not be recognizable.',
        'video': 'Unable to extract audio or text from the video. The video may not contain an audio track, the audio may be unclear, or there may be no usable content.',
        'text': 'The provided text is empty or too short to analyze.',
        'url': 'Unable to extract content from the URL. The page may be inaccessible or contain no analyzable content.'
    }
    return messages.get(input_type, 'Unable to extract content from the provided input.')
...
def extract_content_from_input(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if input_type == 'text':
            ...
            # Check if deep analysis mode (deepfake detection only)
            if input_data.get('deep_analysis', False):
                logger.info("Image deep analysis mode - running deepfake detection only")
                try:
                    import tempfile
                    import os
                    temp_path = None
                    try:
                        ...
                        # Use production image detector (ViT v2 - 100% tested accuracy)
                        from deepfake_detector import detect_image_deepfake
                        deepfake_result = detect_image_deepfake(temp_path)

                        extraction_result['deepfake_analysis'] = deepfake_result
                        extraction_result['text'] = ""  # Empty to trigger media-only mode
                        extraction_result['metadata']['analysis_mode'] = 'image_deepfake_only'
                        extraction_result['metadata']['skip_text_analysis'] = True
                        logger.info(f"Image deepfake analysis complete: {deepfake_result.get('is_deepfake', 'unknown')}")
                    finally:
                        if temp_path and os.path.exists(temp_path):
                            os.unlink(temp_path)
                except Exception as deepfake_error:
                    logger.warning(f"Deepfake detection failed: {deepfake_error}")
                ...
                # Normal mode - extract text from image for claim analysis
                try:
                    text, confidence, analysis = extract_text_from_any_content(
                        image_data, 'image', input_data['metadata']
                    )

                    # Check if text is empty or too short (less than 3 words)
                    word_count = len(text.strip().split()) if text else 0
                    if word_count < 3:
                        logger.info(f"Image contains no meaningful text ({word_count} words) - suggesting Deep Analysis")

                        # Instead of auto-switching, provide a friendly message suggesting Deep Analysis
                        extraction_result['text'] = "No text found in the image."
                        extraction_result['confidence'] = 0.0
                        extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                        extraction_result['metadata']['user_message'] = 'No text found in the image. If you want to check if this image is a deepfake, please enable "Deep Analysis" mode.'
                        extraction_result['metadata']['suggest_deep_analysis'] = True
                    else:
                        # Text found - proceed with normal claim analysis
                    ...
                    # Suggest Deep Analysis instead of auto-switching
                    logger.info("Image text extraction failed - suggesting Deep Analysis")

                    extraction_result['text'] = "Unable to extract text from this image."
                    extraction_result['confidence'] = 0.0
                    extraction_result['metadata'] = {
                        'extraction_method': 'failed_gracefully',
                        'original_error': sanitize_error_message(str(extract_error)),
                        'user_message': 'Unable to extract text from this image. The image may be blank, contain only graphics, or the text may not be clear. If you want to check if this image is a deepfake, please enable "Deep Analysis" mode.',
                        'suggest_deep_analysis': True
                    }
            ...
            # Check if deep analysis mode (deepfake detection only)
            if input_data.get('deep_analysis', False):
                logger.info("Audio deep analysis mode - running deepfake detection only")
                try:
                    import tempfile
                    import os
                    temp_path = None
                    try:
                        ...
                        # Use production audio detector (forensic analysis)
                        from deepfake_detector import detect_audio_deepfake
                        deepfake_result = detect_audio_deepfake(temp_path)

                        extraction_result['deepfake_analysis'] = deepfake_result
                        extraction_result['text'] = ""  # Empty to trigger media-only mode
                        extraction_result['metadata']['analysis_mode'] = 'audio_deepfake_only'
                        extraction_result['metadata']['skip_text_analysis'] = True
                        logger.info(f"Audio deepfake analysis complete: {deepfake_result.get('is_deepfake', 'unknown')}")
                    finally:
                        if temp_path and os.path.exists(temp_path):
                            os.unlink(temp_path)
                except Exception as deepfake_error:
                ...
                # Normal mode - transcribe audio for claim analysis
                try:
                    text, confidence, analysis = extract_text_from_any_content(
                        audio_data, 'audio', input_data['metadata']
                    )

                    # Check if transcription is empty or too short (less than 3 words)
                    word_count = len(text.strip().split()) if text else 0
                    if word_count < 3:
                        logger.info(f"Audio contains no speech ({word_count} words) - suggesting Deep Analysis")

                        # Instead of auto-switching, provide a friendly message suggesting Deep Analysis
                        extraction_result['text'] = "No speech detected in the audio."
                        extraction_result['confidence'] = 0.0
                        extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                        extraction_result['metadata']['user_message'] = 'No speech detected in the audio. The audio may be silent, unclear, or contain only background noise. If you want to check if this audio is a deepfake, please enable "Deep Analysis" mode.'
                        extraction_result['metadata']['suggest_deep_analysis'] = True
                    else:
                        # Speech found - proceed with normal claim analysis
                        extraction_result['text'] = text
                        extraction_result['confidence'] = confidence
                        extraction_result['metadata'].update(analysis)
                    ...
                    # Suggest Deep Analysis instead of auto-switching
                    logger.info("Audio transcription failed - suggesting Deep Analysis")

                    extraction_result['text'] = "Unable to transcribe speech from this audio."
                    extraction_result['confidence'] = 0.0
                    extraction_result['metadata'] = {
                        'extraction_method': 'failed_gracefully',
                        'original_error': sanitize_error_message(str(extract_error)),
                        'audio_size': len(audio_data),
                        'user_message': 'Unable to transcribe speech from this audio. The audio may be silent, unclear, corrupted, or contain only background noise. If you want to check if this audio is a deepfake, please enable "Deep Analysis" mode.',
                        'suggest_deep_analysis': True
                    }
            ...
            # Check if deep analysis mode (deepfake detection only)
            if input_data.get('deep_analysis', False):
                logger.info("Video deep analysis mode - running deepfake detection only")
                try:
                    import tempfile
                    import os
                    temp_path = None
                    try:
                        ...
                        # Use production video detector (ViT v2 frame analysis)
                        from deepfake_detector import detect_video_deepfake
                        deepfake_result = detect_video_deepfake(temp_path)
                    ...
                    extraction_result['text'] = "Unable to analyze this video."
                    extraction_result['confidence'] = 0.0
                    extraction_result['metadata']['analysis_mode'] = 'video_deepfake_only'
                    extraction_result['metadata']['skip_text_analysis'] = True
                    extraction_result['metadata']['error'] = True
                    extraction_result['metadata']['user_message'] = 'Unable to perform deepfake detection on this video. The video may be corrupted, too dark, lack usable content, or be in an unsupported format.'
            else:
                # Normal mode - extract audio from video and transcribe for claim analysis
                logger.info("Video normal mode - extracting audio and transcribing speech")
                try:
                    import tempfile
                    import os
                    ...
                    try:
                        ...
                        try:
                            import ffmpeg
                            audio_temp_path = video_temp_path.replace('.mp4', '_audio.wav')

                            # Extract audio track and convert to WAV format for Speech API
                            stream = ffmpeg.input(video_temp_path)
                            stream = ffmpeg.output(stream, audio_temp_path,
                                                 acodec='pcm_s16le',  # PCM 16-bit format
                                                 ac=1,                 # Mono channel
                                                 ar='16000')           # 16kHz sample rate (optimal for speech)
                            ffmpeg.run(stream, capture_stdout=True, capture_stderr=True, overwrite_output=True)

                            logger.info(f"Audio extracted from video successfully: {audio_temp_path}")

                            # Read extracted audio
                            with open(audio_temp_path, 'rb') as audio_file:
                                audio_data = audio_file.read()

                            # Transcribe audio using existing audio processing
                            text, confidence, analysis = extract_text_from_any_content(
                                audio_data, 'audio', input_data['metadata']
                            )
                            ...
                            word_count = len(text.strip().split()) if text else 0
                            if word_count < 3:
                                logger.info(f"Video contains no speech ({word_count} words) - suggesting Deep Analysis")

                                # Suggest Deep Analysis instead of auto-switching
                                extraction_result['text'] = "No speech detected in the video."
                                extraction_result['confidence'] = 0.0
                                extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                                extraction_result['metadata']['user_message'] = 'No speech detected in the video. The video may have no audio, be silent, or contain only background noise. If you want to check if this video is a deepfake, please enable "Deep Analysis" mode.'
                                extraction_result['metadata']['suggest_deep_analysis'] = True
                            else:
                                # Speech found - proceed with normal claim analysis
                                extraction_result['text'] = text
                                extraction_result['confidence'] = confidence
                                extraction_result['metadata'].update(analysis)
                                extraction_result['metadata']['extraction_method'] = 'video_audio_transcription'
                                logger.info(f"Video audio transcription successful: {word_count} words extracted")

                        except ImportError:
                            logger.warning("ffmpeg-python not available - cannot extract audio from video")
                            extraction_result['text'] = ""
                            extraction_result['confidence'] = 0.0
                            extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                            extraction_result['metadata']['user_message'] = 'Video audio extraction is not available. If you want to check if this video is a deepfake, please enable "Deep Analysis" mode.'
                            extraction_result['metadata']['suggest_deep_analysis'] = True

                        except Exception as ffmpeg_error:
                            logger.warning(f"Audio extraction from video failed: {ffmpeg_error}")
                            extraction_result['text'] = ""
                            extraction_result['confidence'] = 0.0
                            extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                            extraction_result['metadata']['original_error'] = sanitize_error_message(str(ffmpeg_error))
                            extraction_result['metadata']['user_message'] = 'Unable to extract audio from this video. The video may have no audio track, be corrupted, or in an unsupported format. If you want to check if this video is a deepfake, please enable "Deep Analysis" mode.'
                            extraction_result['metadata']['suggest_deep_analysis'] = True
                    ...
                    logger.warning(f"Video audio extraction failed: {extract_error}")
                    extraction_result['text'] = "Unable to extract audio from this video."
                    extraction_result['confidence'] = 0.0
                    extraction_result['metadata'] = {
                        'extraction_method': 'failed_gracefully',
                        'original_error': sanitize_error_message(str(extract_error)),
                        'video_size': len(video_data),
                        'user_message': 'Unable to extract and transcribe audio from this video. The video may be corrupted or in an unsupported format. If you want to check if this video is a deepfake, please enable "Deep Analysis" mode.',
                        'suggest_deep_analysis': True
                    }
        ...
        has_error = extraction_result['metadata'].get('error', False)

        # For media-only mode, empty text is expected - skip validation
        if not is_media_only and not has_error:
            ...
            if (not extraction_result['text'] or
                ...
                # Instead of raising an error, provide a user-friendly fallback message
                logger.warning("No meaningful content extracted - providing user-friendly message")
                extraction_result['text'] = "Unable to extract text from this media."
                extraction_result['confidence'] = 0.0
                extraction_result['metadata']['extraction_method'] = 'failed_gracefully'
                extraction_result['metadata']['user_message'] = get_friendly_extraction_error_message(input_type)

        logger.info(f"Content extraction completed: {len(extraction_result['text'])} characters (media-only: {is_media_only}, has_error: {has_error})")
...
def analyze_claim_comprehensive(extracted_content: Dict[str, Any], input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check if extraction failed and return user-friendly message
        if extracted_content.get('metadata', {}).get('error', False):
            user_message = extracted_content['metadata'].get('user_message', 'Unable to analyze the provided content.')
            logger.warning(f"Extraction failed for claim {claim_id}, returning user-friendly message")
            return {
                'verdict': 'Unable to Analyze',
                'score': 0,
                'explanation': user_message,
                'explainLikeAKid': 'We could not analyze this content because we could not extract any information from it.',
                'sources': [],
                'categories': {},
        ...
        # Check if extraction method was failed_gracefully (no content found)
        if extracted_content.get('metadata', {}).get('extraction_method') == 'failed_gracefully':
            user_message = extracted_content['metadata'].get('user_message', 'No content could be extracted from the media.')
            logger.warning(f"No content extracted for claim {claim_id}, returning user-friendly message")
            return {
                'verdict': 'No Content Found',
                'score': 0,
                'explanation': user_message,
                'explainLikeAKid': 'We could not find any text or speech in the media you provided.',
                'sources': [],
                'categories': {},
                'processing_time': time.time() - analysis_start_time,
                'claim_id': claim_id,
...
def handle_analyze_request(request: Request):
    ...
    try:
        ...
        # SPEED OPTIMIZATION: Check cache for media files (instant response if cached)
        cache_key = None
        if input_data['type'] in ['image', 'audio', 'video']:
            try:
                # Generate cache key from file content
                file_content = base64.b64decode(input_data['content'])
                cache_key = f"{input_data['type']}_{get_file_hash(file_content)}"

                # Check cache
                cached_result = get_cached_response(cache_key)
                if cached_result:
                    return add_cors_headers(jsonify(cached_result)), 200
        ...
        if is_media_deepfake_only:
            # ENHANCED: Deep Analysis Mode - Return ONLY deepfake results, NO claim verification
            if analysis_mode == 'audio_deepfake_only':
                media_type = "Audio"
            elif analysis_mode == 'video_deepfake_only':
                media_type = "Video"
            else:
                media_type = "Image"
            logger.info(f"{media_type} Deep Analysis mode - returning ONLY deepfake detection results")

            deepfake_data = extracted_content.get('deepfake_analysis', {})

            # Check if deepfake detection failed and return user-friendly message
            if deepfake_data.get('error') or extracted_content.get('metadata', {}).get('error', False):
                user_message = extracted_content['metadata'].get('user_message', f'Unable to perform deepfake detection on this {media_type.lower()}.')
                logger.warning(f"{media_type} deepfake detection failed, returning user-friendly message")
                analysis_result = {
                    'success': False,
                    'mode': 'deep_analysis',
                ...
                # Determine if deepfake was detected (check both possible keys)
                is_deepfake = deepfake_data.get('is_deepfake', False) or deepfake_data.get('deepfake_detected', False)
                is_authentic = deepfake_data.get('is_authentic', True)

                # Ensure consistency: if not authentic, it's a deepfake
                if not is_authentic:
                    is_deepfake = True

                # Create MINIMAL response with ONLY binary deepfake result
                # Hide: confidence, risk scores, technical details, indicators, methods
                analysis_result = {
                    'success': True,
                    'mode': 'deep_analysis',
                    'media_type': media_type.lower(),

                    # BINARY RESULT ONLY: Deepfake or Not Deepfake
                    'is_deepfake': is_deepfake,
                    'result': 'Deepfake' if is_deepfake else 'Not Deepfake',

                    # Simple message
                    'message': f'{media_type} deepfake analysis complete.',
            ...
            # Add auto-fallback notification if applicable (for transparency)
            if extracted_content.get('metadata', {}).get('auto_fallback'):
                analysis_result['note'] = 'No text detected - analyzed for deepfake instead.'

            # Calculate total processing time for deep analysis mode
            ...
            # Add metadata (only for normal mode, not Deep Analysis)
            total_processing_time = time.time() - request_start_time
        ...
        # SPEED OPTIMIZATION: Cache successful results for faster future responses
        if cache_key:
            try:
